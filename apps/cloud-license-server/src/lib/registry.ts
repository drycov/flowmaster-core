...
import { hashLicenseKey, parseLicenseKey } from "./keys.js";
import { persistUsageTelemetry, sanitizeUsageTelemetry } from "./telemetry.js";
import type {
...
async function activateKeyForInstallation(
  ...
  if (prior) {
    ...
    const { error: insErr } = await supabase.from("license_server_activations").insert({
      key_id: keyId,
...
export async function upsertProvisionOnServer(
  ...
  return data as { id: string; installation_id: string };
}
...
export async function revokeOnLicenseServer(
  ...
  if (req.key_id || req.key_hash) {
    let q = supabase.from("license_server_keys").update({
      status: "revoked",
      revoked_at: now,
...
export function verifyLicenseServerAdmin(authorization: string | undefined): boolean {
