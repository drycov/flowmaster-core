...
| `--license-server-url=` | Переопределить z-license URL (default `https://z-license.vercel.app`) |
| `--installation-id=` | UUID из кабинета z-license |
| `--with-license-server` | Online-клиент EDMS → z-license |
...
| `npm run license:support-code` | Support code для Console (Docker vendor replica) |
