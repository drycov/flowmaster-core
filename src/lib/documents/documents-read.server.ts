...
import { DOCUMENT_FULL_BASE_SELECT } from "@/lib/api/documents.shared.server";
...
export async function enrichDocumentListRows<T extends { document_type_id?: string | null }>(
  client: SupabaseClient,
  rows: T[],
): Promise<(T & { ref_document_types: { code: string } | null })[]> {
  const typeIds = [...new Set(rows.map((r) => r.document_type_id).filter(Boolean))] as string[];
  if (typeIds.length === 0) {
    return rows.map((r) => ({ ...r, ref_document_types: null }));
  }
  ...
  return rows.map((r) => ({
    ...r,
    ref_document_types: r.document_type_id ? (byId.get(r.document_type_id) ?? null) : null,
  }));
}
